...
Графический интерфейс - версия 6.0 с правильным порядком
"""
...
class FileRenamerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Переименование файлов по таблице v6.0")
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(
            self.root, 
            text="📁 Переименование файлов по таблице v6.0",
            style='Title.TLabel'
        )
        ...
        info_label = tk.Label(
            info_frame,
            text="📋 ФОРМАТ РАБОТЫ:\n" +
                 "1. Файлы в папке сортируются ПО ДАТЕ СОЗДАНИЯ (от старых к новым)\n" +
                 "2. Первый файл (самый старый) → первое имя из таблицы\n" +
                 "3. Второй файл → второе имя из таблицы, и т.д.\n" +
                 "4. Дубликаты имен получают номера (1), (2)...\n" +
    ...
    def get_file_creation_time(self, file_path):
        ...
        try:
            ...
            self.log(f"⚠️  Ошибка получения времени создания {file_path.name}: {str(e)}")
            return 0
    ...
    def start_renaming(self):
        ...
        confirm = messagebox.askyesno(
            ...
            "📋 ПОРЯДОК РАБОТЫ:\n"
            "1. Файлы сортируются по дате создания (от старых к новым)\n"
            "2. Первый файл (самый старый) → первое имя из таблицы\n"
            "3. Второй файл → второе имя из таблицы\n"
        ...
        self.log("\n" + "="*70)
        self.log("🚀 НАЧАЛО ПЕРЕИМЕНОВАНИЯ ФАЙЛОВ (Версия 6.0)")
        self.log("="*70)
        ...
        try:
            ...
            # Показываем все действительные имена в порядке таблицы
            self.log("\n📝 ИМЕНА ИЗ ТАБЛИЦЫ (в порядке строк):")
            ...
            files.sort(key=lambda x: self.get_file_creation_time(x))
            ...
            # Показываем порядок файлов после сортировки по дате
            self.log("\n📋 ФАЙЛЫ ОТСОРТИРОВАНЫ ПО ДАТЕ СОЗДАНИЯ:")
            self.log("(от самых старых к самым новым)")
            
            # Создаем список для сопоставления
            file_order = []
            ...
            for i, file_path in enumerate(files):
                ...
                if i < 10 or i >= len(files) - 5:  # Первые 10 и последние 5
                    self.log(f"  [{i+1:3d}] {file_path.name:35} (создан: {date_str})")
                elif i == 10:
                    self.log(f"  ... (пропущено {len(files) - 15} файлов) ...")
            
            # 3. Сравниваем количества
            self.log("\n⚖️  ПЛАН ПЕРЕИМЕНОВАНИЯ:")
            self.log("-" * 50)
            ...
            self.log(f"Имен в таблице: {names_count}")
            ...
            self.log("-" * 50)
            self.log("Формат: [Порядковый номер] Старое имя → Новое имя (Дата создания)")
            ...
            for i in range(files_to_rename):
                ...
                try:
                    ...
                    self.log(f"✅ [{i+1:3d}] {old_name:30} → {final_name_mp4} ({file_info['date_str']})")
                    success_count += 1
                except Exception as e:
            ...
            if files_count > names_count:
                ...
                for i in range(names_count, files_count):
                    if i < names_count + 10:  # Показываем только первые 10
                        file_info = file_order[i]
                        self.log(f"  [{i+1:3d}] {file_info['old_name']} (создан: {file_info['date_str']})")
                if skipped_count > 10:
            ...
            if analysis['total_rows'] > names_count:
                self.log(f"  ℹ️  Пустых строк в таблице: {analysis['total_rows'] - names_count}")
            
            # Сводка по порядку
            self.log("\n📋 СВОДКА ПО ПОРЯДКУ:")
            self.log(f"  1. Файлы отсортированы по дате создания")
            ...
            if skipped_count > 0:
                result_parts.append(f"\n⏹️  Пропущено: {skipped_count} файлов (не хватило имен в таблице)")
            
            if duplicates:
                result_parts.append(f"\n📝 Дубликаты имен: {len(duplicates)}")
            
            result_parts.append("\n\n📋 ПОРЯДОК РАБОТЫ:")
            result_parts.append("\n1. Файлы отсортированы по дате создания")
