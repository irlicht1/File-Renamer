...
Графический интерфейс - версия 9.1 (без выбора заголовка)
"""
...
class FileRenamerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Переименование файлов по таблице v9.1")
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(
            self.root, 
            text="📁 Переименование файлов по таблице v9.1",
            style='Title.TLabel'
        )
        ...
        info_label = tk.Label(
            ...
            text="📋 ФОРМАТ РАБОТЫ:\n" +
                 ...
                 "3. ✅ ТАБЛИЦА ЧИТАЕТСЯ БЕЗ ЗАГОЛОВКА (данные с первой строки)\n" +
                 "4. Имена берутся из ПЕРВОГО СТОЛБЦА таблицы",
            font=('Arial', 9),
            bg="#f0f8ff",
            justify="left",
            padx=10,
        ...
        examples_label = tk.Label(
            examples_frame,
            text="📝 ПРИМЕР ТАБЛИЦЫ (читается БЕЗ заголовка):\n" +
                 "A1='видео1', A2='видео2', A3='видео3' (и так далее)\n" +
                 "❗ Первая строка таблицы = первое имя файла",
            font=('Arial', 8),
            bg="#fff0f0",
            justify="left",
            padx=5,
    ...
    def start_renaming(self):
        ...
        confirm = messagebox.askyesno(
            ...
            "2. Первый файл → первое имя из таблицы\n"
            "3. Таблица читается БЕЗ ЗАГОЛОВКА (данные с первой строки)\n"
            "4. Имена берутся из ПЕРВОГО СТОЛБЦА\n"
            "5. К именам добавляется .mp4\n\n"
            "⚠️  ВАЖНО: Рекомендуется сделать резервную копию файлов перед началом."
        )
        ...
        self.log("\n" + "="*70)
        self.log("🚀 НАЧАЛО ПЕРЕИМЕНОВАНИЯ ФАЙЛОВ (Версия 9.1)")
        self.log("="*70)
        ...
        try:
            ...
            self.log("\n📊 ЗАГРУЗКА ТАБЛИЦЫ:")
            self.log("-" * 50)
            self.log("Режим: БЕЗ заголовка (данные читаются с первой строки)")
            
            # Всегда читаем таблицу без заголовка
            header_setting = None
            ...
            if files_count != names_count:
                ...
                if files_count > names_count:
                    ...
                    self.log(f"\n💡 ВОЗМОЖНЫЕ ПРИЧИНЫ:")
                    self.log(f"  1. В таблице недостаточно имен")
                    self.log(f"  2. В таблице есть пустые строки")
                    self.log(f"  3. В таблице есть строки только с пробелами")
                else:
            ...
            self.log(f"  📋 Действительных имен в таблице: {names_count}")
            self.log(f"  📋 Режим чтения таблицы: БЕЗ заголовка (данные с первой строки)")
            
            if skipped_count > 0:
                self.log(f"\n💡 РЕКОМЕНДАЦИИ:")
                self.log(f"  1. Проверьте, что в таблице достаточно имен")
                self.log(f"  2. Убедитесь, что в таблице нет пустых строк в начале")
                self.log(f"  3. Убедитесь, что имена находятся в первом столбце (столбец A)")
            ...
            if skipped_count > 0:
                ...
                result_parts.append(f"\n\n💡 РЕКОМЕНДАЦИЯ:")
                result_parts.append(f"\nПроверьте таблицу:")
                result_parts.append(f"\n1. Достаточно ли имен в таблице")
                result_parts.append(f"\n2. Нет ли пустых строк в начале таблицы")
                result_parts.append(f"\n3. Все ли имена в первом столбце (столбец A)")
