...
Графический интерфейс - сортировка по дате создания
"""
...
class FileRenamerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Переименование файлов по таблице v4.0")
    ...
    def get_file_creation_time(self, file_path):
        ...
        try:
            ...
            self.log(f"⚠️  Ошибка получения времени создания файла {file_path.name}: {str(e)}")
    ...
    def start_renaming(self):
        ...
        confirm = messagebox.askyesno(
            ...
            "ВАЖНО:\n"
            "1. Файлы в папке будут отсортированы ПО ДАТЕ СОЗДАНИЯ (от старых к новым)\n"
        ...
        try:
            ...
            # 2. Получаем список файлов в папке и сортируем по дате создания
            ...
            self.log("📋 Порядок файлов (по дате создания, от старых к новым):")
            
            # Показываем первые 5 файлов с их датами создания
            for i, file_path in enumerate(files[:5]):
                ...
                self.log(f"  {i+1}. {file_path.name} (создан: {date_str})")
