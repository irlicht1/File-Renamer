...
Графический интерфейс - версия 5.0 с улучшенной диагностикой
"""
...
class FileRenamerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Переименование файлов по таблице v5.0")
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(
            self.root, 
            text="📁 Переименование файлов по таблице v5.0",
            style='Title.TLabel'
        )
        ...
        info_label = tk.Label(
            ...
            text="Формат таблицы: один столбец с новыми именами (без расширения)\n" +
                 "Порядок: файлы в папке сортируются ПО ДАТЕ СОЗДАНИЯ (от старых к новым)\n" +
                 "Дубликаты: одинаковые имена получат номера (1), (2) и т.д.\n" +
                 "Важно: пустые строки в таблице игнорируются",
            font=('Arial', 9),
            bg="#f0f8ff",
            justify="left",
            padx=10,
    ...
    def start_renaming(self):
        ...
        confirm = messagebox.askyesno(
            ...
            "3. К именам добавляется .mp4\n"
            "4. Дубликаты получают номера (1), (2)...\n"
            "5. Пустые строки в таблице игнорируются\n\n"
        ...
        self.log("\n" + "="*70)
        self.log("🚀 НАЧАЛО ПЕРЕИМЕНОВАНИЯ ФАЙЛОВ")
        self.log("="*70)
        ...
        try:
            ...
            self.log("\n📝 ДЕЙСТВИТЕЛЬНЫЕ ИМЕНА ИЗ ТАБЛИЦЫ:")
            for i, name in enumerate(analysis['valid_names'].head(20)):
                self.log(f"  {i+1:3d}. '{name}'")
            ...
            self.log("\n📋 ФАЙЛЫ В ПОРЯДКЕ ДАТЫ СОЗДАНИЯ:")
            
            # Показываем все файлы с датами
            for i, file_path in enumerate(files):
                if i < 10 or i >= len(files) - 5:  # Первые 10 и последние 5
                    ...
                    self.log(f"  {i+1:3d}. {file_path.name:40} (создан: {date_str})")
                elif i == 10:
            ...
            if files_count != names_count:
                ...
                if files_count > names_count:
                    self.log(f"⚠️  Не хватит имен для {diff} файлов")
                else:
                    self.log(f"⚠️  Останется {diff} неиспользованных имен")
            ...
            for i, file_path in enumerate(files):
                ...
                try:
                    ...
                    creation_time = self.get_file_creation_time(new_path)
                    date_str = datetime.fromtimestamp(creation_time).strftime('%Y-%m-%d %H:%M')
                    self.log(f"✅ {i+1:03d}: {old_name:30} → {final_name_mp4} ({date_str})")
                    success_count += 1
                except Exception as e:
                    self.log(f"❌ {i+1:03d}: Ошибка переименования {old_name} → {str(e)[:50]}...")
                    error_count += 1
            ...
            self.log(f"  Пропущено (не хватило имен): {skipped_count} файлов")
            ...
            if analysis['total_rows'] > names_count:
                self.log(f"  Пустых/недействительных строк в таблице: {analysis['total_rows'] - names_count}")
            
            self.log("="*70)
            
            # Обновляем статус
            status_msg = f"Готово! Успешно: {success_count}, Ошибок: {error_count}"
            if skipped_count > 0:
            ...
            result_parts = []
            result_parts.append(f"Переименование завершено!")
            ...
            if skipped_count > 0:
                result_parts.append(f"\n⏹️  Пропущено: {skipped_count} файлов (не хватило имен)")
            
            if duplicates:
                result_parts.append(f"\n📝 Дубликаты: {len(duplicates)} имён")
                dup_list = list(duplicates.items())
                for name, count in dup_list[:3]:
                    result_parts.append(f"\n- '{name}': {count} раза")
                if len(dup_list) > 3:
                    result_parts.append(f"\n... и ещё {len(dup_list) - 3}")
            
            if analysis['total_rows'] > names_count:
                result_parts.append(f"\nℹ️  В таблице было {analysis['total_rows']} строк, но только {names_count} действительных имен")
