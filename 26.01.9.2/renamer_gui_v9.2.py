...
Графический интерфейс - версия 9.2 с исправлением проблемы дублирующих имен
"""
...
class FileRenamerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Переименование файлов по таблице v9.2")
        ...
        self.has_header = tk.BooleanVar(value=True)  # По умолчанию есть заголовок
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(
            self.root, 
            text="📁 Переименование файлов по таблице v9.2",
            style='Title.TLabel'
        )
        ...
        info_label = tk.Label(
            ...
            text="📋 ФОРМАТ РАБОТЫ:\n" +
                 ...
                 "3. Автоматическое определение заголовка Excel\n" +
                 "4. ✅ ВАЖНО: Выберите правильную настройку заголовка",
            font=('Arial', 9),
            bg="#f0f8ff",
            justify="left",
            padx=10,
        ...
        # Фрейм для таблицы
        table_frame = ttk.LabelFrame(self.root, text="1. Выберите таблицу и настройки")
        ...
        ttk.Checkbutton(
            table_frame,
            text="В таблице ЕСТЬ строка заголовка (первая строка - название столбца)",
        ...
        examples_frame.grid(row=2, column=0, columnspan=3, padx=5, pady=5, sticky="ew")
        
        examples_label = tk.Label(
            examples_frame,
            text="Примеры:\n" +
                 "✅ ЕСТЬ заголовок: A1='Имена', A2='видео1', A3='видео2' (ставим галочку)\n" +
                 "✅ НЕТ заголовка: A1='видео1', A2='видео2', A3='видео3' (снимаем галочку)",
            font=('Arial', 8),
            bg="#fff0f0",
            justify="left",
            padx=5,
    ...
    def detect_excel_header(self, file_path):
        """Автоматическое определение наличия заголовка в Excel"""
        try:
            # Читаем файл без заголовка, чтобы увидеть первую строку как данные
            df_no_header = pd.read_excel(file_path, header=None, nrows=5)
            
            if len(df_no_header) == 0:
                return True  # По умолчанию
            
            first_row = df_no_header.iloc[0, 0] if len(df_no_header.columns) > 0 else None
            
            # Эвристики для определения заголовка:
            # 1. Если первая ячейка пустая или NaN - вероятно, есть заголовок
            # 2. Если первая ячейка содержит типичные названия столбцов
            # 3. Если первая ячейка не похожа на имя файла
            
            if pd.isna(first_row):
                return True
            
            first_str = str(first_row).strip().lower()
            
            # Типичные названия столбцов
            header_keywords = ['название', 'имя', 'name', 'filename', 'file', 'video', 
                             'видео', 'заголовок', 'title', 'header', 'имена файлов']
            
            # Проверяем, похоже ли на заголовок
            if any(keyword in first_str for keyword in header_keywords):
                return True
            
            # Проверяем, похоже ли на имя файла (содержит расширение)
            if '.' in first_str and len(first_str.split('.')[-1]) <= 5:
                # Похоже на имя файла с расширением
                return False
            
            # По умолчанию считаем, что заголовок есть
            return True
            
        except Exception as e:
            self.log(f"⚠️  Ошибка определения заголовка: {str(e)}")
            return True  # По умолчанию
    
    def analyze_table_content(self, new_names_series, header_info=""):
    ...
    def start_renaming(self):
        ...
        # Проверяем настройку заголовка
        has_header = self.has_header.get()
        
        # Подтверждение с информацией о заголовке
        header_status = "ЕСТЬ (первая строка - название столбца)" if has_header else "НЕТ (данные с первой строки)"
        ...
        confirm = messagebox.askyesno(
            ...
            f"Заголовок в таблице: {header_status}\n\n"
            "📋 ПОРЯДОК РАБОТЫ:\n"
            "1. Файлы сортируются ПО АЛФАВИТУ (A-Z, А-Я)\n"
            "2. Первый файл → первое имя из таблицы\n"
        ...
        self.log("\n" + "="*70)
        self.log("🚀 НАЧАЛО ПЕРЕИМЕНОВАНИЯ ФАЙЛОВ (Версия 9.2)")
        self.log("="*70)
        ...
        try:
            ...
            # 1. Загружаем таблицу с учетом настроек заголовка
            self.log("\n📊 ЗАГРУЗКА ТАБЛИЦЫ:")
            self.log("-" * 50)
            
            if has_header:
                self.log("Режим: Строка заголовка ЕСТЬ (header=0)")
                header_setting = 0
                header_desc = "Есть строка заголовка"
            else:
                self.log("Режим: Строка заголовка НЕТ (header=None)")
                header_setting = None
                header_desc = "Нет строки заголовка"
            
            if table.lower().endswith('.csv'):
                df = pd.read_csv(table, encoding='utf-8', header=header_setting)
            else:
                df = pd.read_excel(table, header=header_setting)
            ...
            analysis = self.analyze_table_content(original_names, header_desc)
            
            # Выводим детальную информацию о таблице
            self.log(f"\n📊 АНАЛИЗ ТАБЛИЦЫ:")
            self.log(f"  Режим чтения: {header_desc}")
            ...
            if analysis['duplicates_original']:
                self.log("\n📝 ДУБЛИКАТЫ В ИСХОДНЫХ ДАННЫХ ТАБЛИЦЫ:")
            ...
            if files_count != names_count:
                ...
                if files_count > names_count:
                    ...
                    if has_header:
                        self.log(f"  1. Возможно, в таблице НЕТ строки заголовка")
                        self.log(f"     → Снимите галочку 'В таблице ЕСТЬ строка заголовка'")
                    else:
                        self.log(f"  1. Возможно, в таблице ЕСТЬ строка заголовка")
                        self.log(f"     → Поставьте галочку 'В таблице ЕСТЬ строка заголовка'")
                    self.log(f"  2. Проверьте пустые строки в таблице")
                    self.log(f"  3. Убедитесь, что в таблице достаточно имен")
                else:
            ...
            # - name_usage_count: сколько раз использовалось каждое исходное имя
            # - final_names_used: какие финальные имена уже использовались (для уникальности)
            name_usage_count = {}
            ...
            for i in range(files_to_rename):
                ...
                # Если такое имя уже использовалось, ищем уникальный вариант
                ...
                while final_name in final_names_used:
                    # Если base_final_name уже использован, добавляем дополнительный суффикс
            ...
            if processed_duplicates:
                ...
                for name, count in processed_duplicates.items():
                    ...
                    final_names_for_base = [n for n in final_names_used if n.startswith(name) and not n.startswith(f"{name} (") or n == name]
            ...
            self.log(f"  📋 Действительных имен в таблице: {names_count}")
            self.log(f"  📋 Режим чтения таблицы: {header_desc}")
            ...
            if skipped_count > 0:
                self.log(f"\n💡 РЕКОМЕНДАЦИИ:")
                if has_header:
                    self.log(f"  Попробуйте снять галочку 'В таблице ЕСТЬ строка заголовка'")
                    self.log(f"  → Возможно, ваши данные начинаются с первой строки")
                else:
                    self.log(f"  Попробуйте поставить галочку 'В таблице ЕСТЬ строка заголовка'")
                    self.log(f"  → Возможно, первая строка - заголовок столбца")
            ...
            if skipped_count > 0:
                result_parts.append(f"\n\n💡 РЕКОМЕНДАЦИЯ:")
                if has_header:
                    result_parts.append(f"\nПопробуйте СНЯТЬ галочку 'В таблице ЕСТЬ строка заголовка'")
                    result_parts.append(f"\n(вероятно, ваши данные начинаются с первой строки таблицы)")
                else:
                    result_parts.append(f"\nПопробуйте ПОСТАВИТЬ галочку 'В таблице ЕСТЬ строка заголовка'")
                    result_parts.append(f"\n(вероятно, первая строка - название столбца)")
