@echo off
chcp 65001 > nul
title File Renamer v9.0 - Детальная диагностика
color 0A
cls

echo ========================================================
echo      ПРОГРАММА ПЕРЕИМЕНОВАНИЯ ФАЙЛОВ v8.0
echo      Версия с детальной диагностикой таблицы
echo ========================================================
echo.
...
echo ========================================================
echo 🎯 ОСОБЕННОСТИ ВЕРСИИ 9.0:
echo.
echo 🔍 ДЕТАЛЬНАЯ ДИАГНОСТИКА ТАБЛИЦЫ:
echo   1. Показывает ВСЕ строки таблицы
echo   2. Отображает, какие строки отбрасываются
echo   3. Указывает причину отбрасывания
echo   4. Показывает невидимые символы
echo.
echo 🔤 АЛФАВИТНАЯ СОРТИРОВКА:
echo   1. Файлы сортируются по алфавиту (A-Z, А-Я)
echo   2. Первый файл (A) → первая строка таблицы
echo   3. Второй файл (B) → вторая строка таблицы
echo   4. Работа с заголовками - поставить галочку если есть
echo.
...
if not exist "renamer_gui_v9.py" (
    echo ❌ ОШИБКА: Файл renamer_gui_v9.py не найден!
    echo.
    echo Убедитесь, что в этой папке есть файл renamer_gui_v9.py
    echo или переименуйте ваш файл в renamer_gui_v9.py
    echo.
    pause
    exit /b 1
)
